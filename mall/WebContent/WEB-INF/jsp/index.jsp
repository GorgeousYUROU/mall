...
<div class="lf1">
    ...
    <div class="lf1_bottom">
        <div class="item_cat lf" id="cat161">
...
var SUCCESS = 1;
$(function(){
	var url = "../goods/categories.do"
	$.getJSON(url,{rootId:161},function(result){
		$("#cat161").empty();
		...
		if(result.state == SUCCESS){
			for(var i = 0 ; i < categories.length; i++){
				...
				if(category.parentId == 0){
					root = $(createParent(category));
					$("#cat161").append(root);
						
				}
				if((category.isParent == 1) && (category.parentId != 0)){
					branch = $(createBranch(category));
					root.last().append(branch);
					branches[category.id+""] = branch;
				}
				if(category.isParent == 0){
					leave=$(createLeaves(category));
					branches[category.parentId+""].last().append(leave);
				}
...
function createParent(category){
	var template = '<div class="cat_header color_2">'+'<span>'+
			' <img src="../images/itemCat/computer_icon1.png" alt=""/>'+'  {name}/1F'+
		    '</span>'+'</div>'+
			'<div class="item_cat_all">'+'</div>';
	var str = template.replace('{name}',category.name);
	return str;
}

function createBranch(category){
	var template = '<p>'+'{name}'+'</p>'+'<ul></ul>';
	var str = template.replace('{name}',category.name);
	return str;
}

function createLeaves(category){
	var template = '<li> <a href="../goods/list.do?categoryId={id}">'+'{name}'+'</a></li>';
	var str = template.replace('{name}',category.name).replace('{id}',category.id);
	return str;
}
</script>
